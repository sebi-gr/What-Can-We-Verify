...
**Schritt 1b abgeschlossen (23.09.2026):** [PR #2](https://github.com/sebi-gr/What-Can-We-Verify/pull/2) ist gemergt; die Reproduktion liegt auf `reproduce-vul4j-18`. Vollständiger Benchmark-Snapshot `07ad7850...`, Eclipse Temurin `8u504-b01` und Maven `3.9.16`. Verwundbarer und gefixter Build erfolgreich. Die zwei unveränderten Tests `WikiServletTest#testNastyDoPost` und `#testDoGet` liefen jeweils ohne Errors oder Skips: verwundbar 2/2 Assertion-Fehler (`/?page=Main&` statt `/Wiki.jsp?page=Main&`), gefixt 2/2 bestanden. Der Originalfix ändert nur `getForwardPage()` von `request.getPathInfo()` auf `"Wiki.jsp"`; die fünf Review-Quelldateien stimmen mit dem verwundbaren vollständigen Benchmark überein. Befehle, Umgebung, Hashes und Grenzen stehen in `resources/reproduce_vul4j18.md`; vollständige Logs/Metadaten unter dem ignorierten `data/pov/VUL4J-18-001/`. Dies reproduziert das Mock-Forwarding-Verhalten, keinen End-to-End-Dateizugriff oder unauthentifizierten Remote-Angriff.
...
Aktueller lokaler Abgleich (23.09.2026): Generator, Umstrukturierung, OpenRouter-/`.env`-Unterstützung, Reasoning-Option und Dokumentation sind über PR #2 in `main` integriert (`5d45c00`). Schritt 1b wird auf `reproduce-vul4j-18` dokumentiert; nur diese Reproduktionsdokumentation ist aktuell uncommitted. `data/VUL4J-18/`, der vollständige Review-Lauf und `data/pov/VUL4J-18-001/` sind vorhanden und ignoriert. Letzter Python-Prüfstand vor PR #2: 14/15 Offline-Tests bestanden, ein Symlink-Test wegen fehlenden Windows-Privilegs ausdrücklich übersprungen. Der neueste Review ist vollständig; der Java-PoV ist differenziell reproduziert.
...
| 1b. Java-PoV an verwundbarer/gefixter Version ausführen | **Erledigt** | Beide Builds erfolgreich; verwundbar 2/2 erwartete Assertion-Fehler, gefixt 2/2 bestanden; Umgebung, Befehle und Logs dokumentiert. |
...
Bereits geprüft: drei Offline-Tests, echter Downloadlauf, neun Datei-Hashes und Constructor-Abgleich mit dem Upstream-Fix-Parent. Das Vorbereitungsmanifest behält korrekt `pov_status: not_run`, weil dieser Export den PoV nicht ausführt. Die separate Reproduktion in `data/pov/VUL4J-18-001/` ist abgeschlossen und in `resources/reproduce_vul4j18.md` dokumentiert.

Für 1b wurde der vollständige Benchmark separat aufgebaut. Der Build war auf `jspwiki-main` und seine erforderlichen Reactor-Module begrenzt; die PoV-Tests und Produktionslogik blieben unverändert. Das differenzielle Mock-Ergebnis belegt die getestete Forwarding-Änderung, aber weder beliebige Dateizugriffe noch unauthentifizierte Ausnutzbarkeit.
...
Schritte 1b und 2 sind technisch abgeschlossen. Als Nächstes Schritt 3: das gespeicherte Finding manuell in prüfbare Aussagen zerlegen und das vorläufige Codebook daran schärfen. Offen vor Schritt 4: finales Schema/Offsets und Retry-Regel. Offen vor Evaluation: Stichprobe, Verifikatoren, Referenzannotation und Metriken.
